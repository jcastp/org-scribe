...
- Learned to use the RAE (Real Academia Española) dictionary for Spanish definitions
...
2. Start typing: ~org-scribe-rae-api-lookup~
...
| Key | Tool           | Speed                            |
|-----+----------------+----------------------------------|
| d   | RAE dictionary | Medium (network)                 |
...
- Fails gracefully when offline (shows an error message)
- There is no free offline Spanish *definition* dictionary to fall back on —
  the RAE's dictionary is copyrighted and has never been released under a
  free licence, which is why an online lookup is the only option here. Note
  the offline thesaurus does give you synonyms and antonyms, which covers a
  good part of why writers reach for the dictionary mid-draft.
...
- Look up any definitions you expect to need before disconnecting
