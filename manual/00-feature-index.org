...
14. [[#integrations][Third-Party Integrations]] - Optional connections to companion packages
...
** org-scribe-planner Integration (~org-scribe-planner-integration~)

An optional bridge module that connects ~org-scribe~ to the companion ~org-scribe-planner~ package for goal-oriented writing plans. The module lives at ~integration/org-scribe-planner-integration.el~ and is *not loaded automatically*. Enable it with:
...
Features provided: (1) auto-load the project's plan file when any project file is opened; (2) push live word counts to the plan after running ~org-scribe/ews-org-count-words~; (3) wire the planner's word count function to read ~WORDCOUNT~ properties from the manuscript; (4) set the planner's project-root function so file pickers start at the current project root; (5) offer plan creation after ~org-scribe-create-novel-project~ / ~org-scribe-create-short-story-project~; (6) add a planner sub-hydra accessible via ~W~ in the main Hydra menu; (7) show today's target/actual in the mode line as ~[W:actual/target]~. Each feature is independently disabable.

*Status*: Fully implemented (requires org-scribe-planner) | *Hydra*: ~W~ sub-menu (hidden from hint) | *Module*: ~integration/org-scribe-planner-integration.el~
...
All 88 features are fully implemented and tested. The package is in active use and stable (version 0.4.0).
...
- *Total Features Documented*: 89 (88 core + 1 optional integration)
