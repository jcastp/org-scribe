...
- Find inline *EDIT*/*NOTE* markers left while drafting
...
Two more searches round out the Search section of the hydra (covered later in this lesson): ~F8 F8 5~ for TODO items and ~F8 F8 6~ for inline =*EDIT*=/=*NOTE*= markers (~org-scribe-search-edits~, which parses Org comment blocks rather than querying properties).
...
1. You'll see a prompt: ~Find PoV character [fuzzy]: ~ (or ~Character (PoV) [substring]: ~ if you use the free-text variant)
...
1. You'll see a prompt: ~Find character [fuzzy]: ~ (or ~Character name [substring]: ~)
2. Start typing: ~Sam~
...
1. You'll see a prompt: ~Find plot thread [fuzzy]: ~ (or ~Plot term [substring]: ~)
...
1. You'll see a prompt: ~Find location [fuzzy]: ~ (or ~Location [substring]: ~)
...
Or use: ~M-x org-scribe-search-todos-recursive~
...
* Search 6: Finding Edit/Note Markers

Find every inline ~*EDIT*~ or ~*NOTE*~ marker across your entire project.

** How This Differs from TODO Search

Search 5 (TODOs) looks for Org TODO *keywords* at the start of headings - things you have decided to *write*. Search 6 finds markers you left *inside* prose you have already written, attached to the sentence that needs work.

Both are searches over the whole project; they simply cover different stages of the work.

** When to Use

- **Mid-draft flagging**: Mark a passage as needing work without breaking your writing flow
- **Revision prep**: Collect every flagged passage across the whole project before a revision pass
- **Continuity notes**: Leave yourself a reminder about something to double-check later

** What Are Edit/Note Markers?

A marker is written inside an Org comment block, right after the passage it refers to:

#+BEGIN_EXAMPLE
Alex walked into the coffee shop.

,#+begin_comment
*NOTE*: check this is the same shop from Chapter 2
,#+end_comment

The confrontation happened too fast.

,#+begin_comment
*EDIT*: scene - needs another beat here before the reveal
,#+end_comment
#+END_EXAMPLE

Don't stop writing to fix the passage - drop the marker and keep going.

The comment block is not optional decoration. It is what keeps markers out of your exported manuscript and out of your word counts. A marker typed as bare text in a paragraph is ordinary prose and *will* appear in your exported book. Lesson 14 covers the convention in full, including the ~edit~ and ~note~ snippets that write the block for you.

An ~*EDIT*~ also carries a category before a ~" - "~ separator (~plot~, ~scene~, ~character~, ~prose~, ~design~ by default), so a revision pass can tackle one kind of problem at a time. ~*NOTE*~ has no category - it is a reminder, not a classified piece of work.

** How to Search

Press: ~F8 F8 6~

Or use: ~M-x org-scribe-search-edits~

This collects every marker in every ~.org~ file below your project root and builds an index grouped by kind and category.
...
,* Edits
,** plot
,*** [[file:novel.org::312][Scene 12]] — novel.org
    Alice's motive here contradicts chapter 1
,** scene
,*** [[file:novel.org::420][Scene 14]] — novel.org
    needs another beat here before the reveal
,** character
,** prose
,** other

,* Notes
,** [[file:novel.org::145][Scene 3]] — novel.org
   check this is the same shop from Chapter 2
#+END_EXAMPLE

Each entry shows the scene it came from and links to the exact line. Empty categories are listed too, so you can see at a glance which kinds of problem are outstanding and which are clear.

** Using Results

- ~RET~ on an entry jumps to that marker in the manuscript
- ~TAB~ folds and unfolds sections - collapse ~Notes~ while working through ~Edits~, or fold everything to see just the category headings
- ~g~ rebuilds the index after making fixes
- ~q~ buries the buffer

Work through a category, deleting each comment block as you deal with it - that is how a marker gets "closed", there is no separate done state - then press ~g~ to confirm the category is empty.

You can also keep the index open beside your manuscript while you work: set ~org-scribe-editing-right-panel~ to ~edits~ and it becomes the right pane of Editing mode, rebuilding itself every time you save (lesson 9).

** Customizing the Markers

The categories come from ~org-scribe-edit-categories~:

#+BEGIN_SRC emacs-lisp
(setq org-scribe-edit-categories '("plot" "scene" "character" "prose" "pacing"))
#+END_SRC

A marker whose category is missing, empty or unrecognized is grouped under ~other~ rather than dropped - a typo moves a marker, it never hides one.

If your markers *don't* follow the comment-block convention - older projects, or a marker style of your own - call the search with a prefix argument (~C-u F8 F8 6~) to get ~org-scribe-search-edits-rgrep~, a plain-text search that finds them anywhere. That fallback is the one that uses ~org-scribe-edit-string~:
...
Search results appear in an org-ql results buffer (named after your query, e.g. ~*Org QL View: ...*~ — the exact name depends on your installed org-ql version)
...
- **Review TODOs**: Check what you left yourself to fix (F8 F8 5)
...
Open the manuscript file directly and check the property with ~C-c C-x p~ (=org-set-property=, which also displays the current value) — there is no dedicated diagnostic command for this.
...
| ~F8 F8 5~ | TODO items        | TODO keywords                |
| ~F8 F8 6~ | Edit/note markers | =*EDIT*=/=*NOTE*= in comment blocks |
...
| ~org-scribe-search-todos-recursive~ | Find all TODO items           |
| ~org-scribe-search-edits~           | Find inline edit/note markers |
...
✓ How to find inline *EDIT*/*NOTE* markers across your project
