...
An ~*EDIT*~ also carries a category before a ~" - "~ separator (~plot~, ~scene~, ~character~, ~prose~ by default), so a revision pass can tackle one kind of problem at a time. ~*NOTE*~ has no category - it is a reminder, not a classified piece of work.
