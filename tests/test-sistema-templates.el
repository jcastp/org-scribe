...
(ert-deftest test-sistema-plot-point-entity-registered ()
  "The plot-point entity type is registered, with its hand-written command.
The macro-generated half of its API is covered by
`test-entity-registry-api-is-generated' below, which checks every
registered entity rather than this one."
  (should (assq 'plot-point org-scribe-entity-registry))
  ;; Not generated by `org-scribe-define-entity' — hand-written in
  ;; linking/org-scribe-plot-point-links.el.
  (should (fboundp 'org-scribe-jump-to-plot-point)))

(ert-deftest test-entity-registry-api-is-generated ()
  "Every function each registered entity declares is actually defined.

`org-scribe-define-entity' records the API an entity declares as :api on
its registry entry, before the per-row :when guards run.  So this
compares what each entity asked for against what the macro emitted, and
catches a guard that silently turns falsy (dropping, say,
`org-scribe-set-scene-characters' from the character API), a spec row
deleted from the generation table, and an entity that declares a key the
table does not emit.

It replaces the per-entity hardcoded `fboundp' lists that used to guard
this, and is stronger than they were: a new entity type added with the
macro is covered here the moment it is registered, with no edit to this
test."
  (should org-scribe-entity-registry)
  (dolist (entry org-scribe-entity-registry)
    (let ((entity (car entry))
          (api (plist-get (cdr entry) :api)))
      ;; Every entity declares an API; an empty one means the macro stopped
      ;; recording it, which would make the loop below vacuously pass.
      (should api)
      (pcase-dolist (`(,name . ,commandp) api)
        (should (fboundp name))
        ;; Rows marked :command must be usable from M-x, since that is how
        ;; the hydra and the manual present them.
        (when commandp
          (should (commandp name))))
      ;; The descriptor the engine functions read must stay a pure config
      ;; plist: :file-fn is what every generated body ultimately calls.
      (should (plist-get (cdr entry) :file-fn))
      (should (symbolp entity)))))
