...
(defconst org-scribe-messages-en
  '(
    ...
    ;; Free text, not a |-alternative list: -1 and an empty answer both have
    ;; to be typable, and alternatives would make the first one the default.
    ...
    ;; Timeline tables — entities hidden by a negative Weight
    (msg-timeline-hidden-entities . "Not shown (Weight -1): %s")
    (msg-weight-hidden-marker . "hidden")

    ;; Manuscript compilation (export/org-scribe-compile.el)
...
(defconst org-scribe-messages-es
  '(
    ...
    ;; Timeline tables — entities hidden by a negative Weight
    ;; Phrasing kept gender-neutral: the same key serves characters
    ;; (personajes, masc.) and plot threads (tramas, fem.).
    (msg-timeline-hidden-entities . "Sin mostrar (Weight -1): %s")
    (msg-weight-hidden-marker . "sin mostrar")

    ;; Compilación del manuscrito (export/org-scribe-compile.el)
    ...
    (compile-done . "Compilados %d capítulo%s y %d escena%s en %s")
