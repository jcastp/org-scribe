...
- *Entity Tooltips* (opt-in): Inline character information without opening a new buffer
  ...
  - Toggle manually with =M-x org-scribe-overlays-mode= or =F8 F8 C T=
- *Word Counting*: Accurate word counting that excludes Org-mode metadata (comments, properties, drawers)
- *Scene-level Word Count Update*: =org-scribe-update-scene-wordcounts= updates =:WORDCOUNT:= on every scene heading in the current subtree without touching chapter or act headings (=F8 F8 z=)
...
*** Add Word Count Properties to All Headings
...
=M-x org-scribe-update-scene-wordcounts= or press =F8 F8 z= in hydra
...
- =F8 F8 C T= — toggle via the characters hydra submenu
...
- =a= - Add word count properties to all headings
- =z= - Update scene word counts in current subtree
- =d= - RAE dictionary
- =y= - Synonyms
...
- =W= - Writing plan submenu (available once the planner loads on first use)
...
Press =F8 F8 W= to open the planner sub-menu (available once the planner has loaded):
