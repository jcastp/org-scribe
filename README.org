...
- [[https://codeberg.org/jcastp/org-tracktable][org-tracktable]] — daily writing-progress tracking.
...
automatically; the two recommended packages are added explicitly because
they live on Codeberg rather than a package archive. Drop them if you
prefer the graceful fallback.

#+begin_src emacs-lisp
;; Recommended companions (Codeberg sources). Omit for graceful fallback.
...
(use-package org-tracktable
  :vc (:url "https://codeberg.org/jcastp/org-tracktable" :rev :newest))
...
*Insert Relationship Graph* (=F8 F8 L g= or =M-x org-scribe-insert-relationship-block=):
...
- =r= - Update org-tracktable progress table
...
- =g= - Insert relationship graph block
...
If you previously installed [[https://codeberg.org/jcastp/org-scribe-planner][org-scribe-planner]] as a standalone package and loaded =integration/org-scribe-planner-integration.el=, you can remove that snippet from your init file. Loading it is harmless (it became a no-op shim), but unnecessary.
