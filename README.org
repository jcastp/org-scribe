...
- *org-scribe-planner Integration* (optional): Connect writing goal planning to your manuscript — auto-loads plans, pushes live word counts, and adds a planner sub-menu at =W=
...
- [[https://codeberg.org/jcastp/org-scribe-planner][org-scribe-planner]] - For writing goal planning (see [[*org-scribe-planner Integration][Integration section]])
...
- =W= - Writing plan submenu (requires org-scribe-planner integration)
...
* org-scribe-planner Integration

** Overview

[[https://codeberg.org/jcastp/org-scribe-planner][org-scribe-planner]] is a companion package for writing goal planning (daily targets, milestones, progress dashboards). The integration module =integration/org-scribe-planner-integration.el= bridges the two packages so that word counts, project roots, and plan files are shared automatically.

This module is *not loaded automatically*. To enable it, add the following to your init file after both packages are loaded:

#+begin_src emacs-lisp
  (with-eval-after-load 'org-scribe
    (require 'org-scribe-planner-integration
             (expand-file-name "integration/org-scribe-planner-integration"
                               (file-name-directory (locate-library "org-scribe")))))
#+end_src
...
When you open any file inside an org-scribe project, the integration looks for a matching plan file and loads it silently. Search order:
...
- After running =org-scribe-ews-org-count-words= (=F8 F8 a=), the active plan is updated automatically and saved silently.

*** 3. Project-Aware File Picker

When loading a plan via =org-scribe-planner-load-plan=, the file picker opens at the current project root instead of the default directory.

*** 4. Plan Creation on Project Creation

After creating a new novel or short story project, you are offered the option to create a matching writing plan immediately via =org-scribe-planner-new-plan=.
...
Each feature is independent. To turn one off:
