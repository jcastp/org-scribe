...
When you enter an org-scribe project, the planner silently looks for a matching plan file and loads it. Search order:
...
- *Not in a project* → falls back to =org-scribe-planner-show-current-plan=.

Every new project includes a =plan.org= placeholder so the planner can find it at step 1 of its search without scanning.

*** 4. Plan Creation on Project Creation

After creating a new novel or short story project, you are offered the option to create a matching writing plan immediately.

*** 5. Planner Hydra Sub-menu (=W=)

Press =F8 F8 W= to open the planner sub-menu:
...
When a plan is active, the mode line shows today's progress: =[W:actual/target]=.

*** 7. Project Health Integration

=M-x org-scribe-project-health= includes a =* Writing Plan= section when the planner is loaded, showing title, target, current word count and percentage, daily target, end date, days remaining, and whether you are on track or behind. When no plan is active a hint to run =M-x org-scribe-plan= is shown instead.
