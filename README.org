...
3. Press =C-c w= (or =<f8> <f8>=) to open the hydra menu
...
Press =C-c w w= or use the hydra menu.
...
Press =C-c w f= or use the hydra menu.
...
=M-x org-context-count-words= or press =C-c w n= in hydra

*** Add Word Count Properties

=M-x writing/ews-org-count-words= or press =C-c w m= in hydra
...
All search functions are available via the hydra menu (=C-c w=):
...
=M-x writing/rae-api-lookup= or press =C-c w r= in hydra

Looks up Spanish words in the Real Academia Española dictionary.

*** Synonyms

=M-x writing/sinonimo= or press =C-c w s= in hydra
