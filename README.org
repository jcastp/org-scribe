...
2. Run =M-x writing-create-novel-project=
...
=M-x writing-create-novel-project=
...
=M-x writing/ews-org-count-words= or press =F8 F8 a= in hydra
...
1. =M-x writing/setup-character-links= or press =F8 F8 C s=
...
When you rename a character in =plan/characters.org=, the ID links still work (they're ID-based), but the display text shows the old name. Update the display names to match your current database:
...
*Example Workflow*:
1. Rename "Alex Rivera" to "Alexandra Rivera" in =plan/characters.org=
2. Press =F8 F8 C U= (or =M-x writing/update-all-character-link-names=)
...
For existing projects:
1. =M-x writing/setup-plot-thread-links= or press =F8 F8 P s=
...
=M-x writing/plot-thread-report= or press =F8 F8 P r=
...
=M-x writing/plot-thread-stats= or press =F8 F8 P S=
...
Note: Like character links, you can update plot thread display names after renaming threads in =plan/plot.org= using =F8 F8 P U= or the unified =F8 F8 U=.
...
=M-x writing/rae-api-lookup= or press =F8 F8 d= in hydra

Looks up Spanish words in the Real Academia Española dictionary.

*** Synonyms

=M-x writing/sinonimo= or press =F8 F8 y= in hydra
...
- =writing/capture-character= (F8 F8 h) - Capture character profiles with comprehensive templates
- =writing/capture-location= (F8 F8 l) - Capture location details for world building
- =writing/capture-object= (F8 F8 b) - Capture important objects in your story
- =writing/capture-timeline= (F8 F8 t) - Capture timeline events for chronology tracking
- =writing/capture-plot-thread= (F8 F8 g) - Capture plot threads as they emerge during writing

These functions automatically route captures to the appropriate file based on project type:
- **Novel projects**: Separate files in =plan/= directory (e.g., =plan/characters.org=, =plan/plot.org=)
...
Projects can be created with =M-x writing-create-novel-project= or the package works with any org-mode file structure.
...
*Proposal 1 Improvements* (v0.2.1):
- *No Shift Keys*: Removed shift requirements from frequent actions (S→s, C→c, O→o)
- *Home Row Optimization*: Best keys for most frequent actions (s=scene, c=chapter, w=words)
- *Better Mnemonics*: Object (j→b), Timeline (k→t), Writing mode (w→m)
- *Numbered Search*: Consistent 1-5 pattern instead of u/i/o/y/a
- *Clear Hierarchy*: Lowercase for actions, capitals ONLY for submenus
...
3. Old function names still work (with deprecation warnings):
   - =writing-project-create-novel-project= → =writing-create-novel-project=
   - =writing-create-project= → =writing-create-novel-project=
